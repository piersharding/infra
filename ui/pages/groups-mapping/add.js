...
export default function AddGroupsMapping() {
  ...
  async function handleSubmit(e) {
    ...
    if (!nameTemplate.trim()) newErrors.name_template = 'Name template is required'
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to save group mapping')

      setNotification({ type: 'success', message: isEdit ? 'Group mapping updated successfully' : 'Group mapping created successfully' })
      
      // Navigate back after a brief delay
      setTimeout(() => router.push('/groups-mapping'), 500)
    } catch (e) {
      setErrors({ submit: e.message || 'Failed to save group mapping' })
    }
  ...
  return (
    <div className='mb-10'>
      <Head>
        <title>{isEdit ? 'Edit Group Mapping' : 'Add Group Mapping Rule'} - Infra</title>
      </Head>
      ...
      <header className='my-6'>
        <Link href='/groups-mapping' className='mb-2 inline-flex items-center text-sm text-gray-500 hover:text-gray-700'>
          <ArrowLeftIcon className='mr-1 h-4 w-4' /> Back to Group Mappings
        </Link>
        <h1 className='py-1 font-display text-xl font-medium'>{isEdit ? 'Edit Rule' : 'Add Group Mapping Rule'}</h1>
      </header>
      ...
      <form onSubmit={handleSubmit} className='space-y-6'>
        ...
        {/* Source Group Regex */}
        <div className='mb-4 flex flex-col'>
          <label htmlFor='source_group_regex' className='text-xs font-medium text-gray-600'>Source Group Regex</label>
          <input
        ...
        {/* Name Template */}
        <div className='mb-4 flex flex-col'>
          <label htmlFor='name_template' className='text-xs font-medium text-gray-600'>Name Template</label>
          <input
            id='name_template'
            name='name_template'
            required
        ...
        <div className='flex items-center justify-end space-x-3 pt-4'>
          <Link href='/groups-mapping' passHref>
