...
// then returns the subset that matches.
...
function AddMappingRuleDialog({ open, setOpen, groups, onMutate }) {
  ...
  const matchedGroups = useMemo(() => {
    ...
    try { return previewRegex(sourceGroupRegex, groups.map(g => g.name)) }
    catch { return [] }
  ...
  async function handleSubmit(e) {
    ...
    else { try { new RegExp(sourceGroupRegex) } catch { newErrors.source_group_regex = 'Invalid regular expression' } }
    if (!nameTemplate.trim()) newErrors.name_template = 'Destination name template is required'
    if (destinationType === 'kubernetes' && !roleTemplate.trim()) newErrors.role_template = 'Role template is required for Kubernetes'

    if (Object.keys(newErrors).length > 0) { setErrors(newErrors); return }

    setSubmitting(true)
    try {
      const body = { rule_name: ruleName.trim(), source_group_regex: sourceGroupRegex, destination_type: destinationType, name_template: nameTemplate }
      if (destinationType === 'kubernetes') body.role_template = roleTemplate
      if (namespaceTemplate && destinationType === 'kubernetes') body.namespace_template = namespaceTemplate

      const res = await fetch('/api/mapping-rules', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      if (!res.ok) {
        const errBody = await res.json().catch(() => null)
        setErrors({ submit: Array.isArray(errBody?.message) ? errBody.message.join(', ') : (errBody?.message || 'Failed to save') })
        return
      }
      await res.json()
      if (onMutate) onMutate()
      setOpen(false)
      setRuleName(''); setSourceGroupRegex(''); setNameTemplate(''); setRoleTemplate(''); setNamespaceTemplate(''); setErrors({})
    } catch (e) {
          ...
          <div className='fixed inset-0 flex items-center justify-center p-4 bg-black/30'>
            <Dialog.Panel className='w-full max-w-lg transform overflow-hidden rounded-xl bg-white p-6 shadow-xl'>
              <div className='flex items-center justify-between mb-4'>
                <h2 className='text-base font-semibold text-gray-900'>Add Mapping Rule</h2>
                <button type='button' onClick={() => setOpen(false)} className='text-gray-400 hover:text-gray-600 cursor-pointer'>&#x2715;</button>
              </div>
              <form onSubmit={handleSubmit} className='space-y-4'>
                {errors.submit && <div className='rounded-md bg-red-50 p-3 text-sm text-red-700'>{typeof errors.submit === 'string' ? errors.submit : JSON.stringify(errors.submit)}</div>}
                <div>
                  <label htmlFor='mr-name' className='text-xs font-medium text-gray-600'>Rule Name</label>
                  <input id='mr-name' value={ruleName} onChange={e => setRuleName(e.target.value)} placeholder='e.g., team-platform-access'
                ...
                <div>
                  <label htmlFor='mr-type' className='text-xs font-medium text-gray-600'>Destination Type</label>
                  <select id='mr-type' value={destinationType} onChange={e => setDestinationType(e.target.value)}
                    className='mt-1 block w-full rounded-md border border-gray-300 shadow-sm sm:text-sm'><option value='kubernetes'>Kubernetes</option><option value='ssh'>SSH</option></select>
                </div>
                <div>
                  <label htmlFor='mr-regex' className='text-xs font-medium text-gray-600'>Group Matching Regex</label>
                  <input id='mr-regex' type='search' value={sourceGroupRegex} onChange={e => setSourceGroupRegex(e.target.value)} placeholder='e.g., ^team-(.*)$'
                ...
                {matchedGroups.length > 0 && <div className='rounded-md bg-green-50 p-2 text-xs'><span className='font-medium'>Matches:</span> {matchedGroups.join(', ')}</div>}
                <div>
                  <label htmlFor='mr-template' className='text-xs font-medium text-gray-600'>Destination Name Template ($N for capture groups)</label>
                  <input id='mr-template' value={nameTemplate} onChange={e => setNameTemplate(e.target.value)} placeholder='e.g., cluster-$1-prod'
                ...
                {destinationType === 'kubernetes' && <>
                  <div>
                    <label htmlFor='mr-role' className='text-xs font-medium text-gray-600'>Role Template</label>
                    <input id='mr-role' value={roleTemplate} onChange={e => setRoleTemplate(e.target.value)} placeholder='e.g., $1-admin'
                  ...
                  <div>
                    <label htmlFor='mr-ns' className='text-xs font-medium text-gray-600'>Namespace Template (optional)</label>
                    <input id='mr-ns' value={namespaceTemplate} onChange={e => setNamespaceTemplate(e.target.value)} placeholder='Optional: e.g., $1-ns'
                      className='mt-1 block w-full rounded-md border border-gray-300 shadow-sm sm:text-sm' />
                  </div>
                </>}
                {destinationType === 'ssh' && <div className='rounded-md bg-yellow-50 p-2 text-xs'>For SSH destinations, the privilege is always "connect" and role template is not used.</div>}
                <div className='flex justify-end space-x-3 pt-4'>
                  <button type='button' onClick={() => setOpen(false)}
                    className='rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium hover:bg-gray-50 cursor-pointer'>Cancel</button>
                  <button type='submit' disabled={submitting} className={`rounded-md border border-transparent bg-black px-4 py-2 text-sm font-medium text-white ${submitting ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-800 cursor-pointer'}`}>
...
export default function GroupsMapping() {
  ...
  return (
    <div className='mb-10'>
      ...
      <header className='my-6'>
        <div className='flex items-center justify-between'>
          <div className='flex flex-1 items-center space-x-4'>
            <h1 className='py-1 font-display text-xl font-medium'>Mapping Rules</h1>
      ...
      {totalCount > 0 && (
        <Table
          data={items.map(m => ({
            ...
            roleTemplatePreview: m.role_template ? previewTemplate(m.role_template, 'team-platform') : '-',
            namespaceTemplatePreview: m.namespace_template ? previewTemplate(m.namespace_template, 'team-platform') : '-',
          }))}
          columns={[
            { header: () => <span>Rule Name</span>, accessorKey: 'ruleName' },
            { header: () => <span>Group Matching Regex</span>, accessorKey: 'sourceGroupRegex' },
            { header: () => <span>Destination Type</span>, accessorKey: 'destinationType' },
      ...
      {/* Add Rule Modal */}
      <AddMappingRuleDialog open={addOpen} setOpen={setAddOpen} groups={groups.map(g => g.name)} onMutate={() => mutate(apiUrl)} />
    </div>
  )
}
