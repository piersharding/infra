...
export default function AddGroupsMapping() {
  ...
  async function handleSubmit(e) {
    ...
    if (destinationType === 'kubernetes' && !roleTemplate.trim()) {
      newErrors.role_template =
        'Role template is required for Kubernetes destinations'
    }
    ...
    try {
      ...
      const url = isEdit
        ? `/api/mapping-rules/${router.query.id}`
        : '/api/mapping-rules'
    ...
    <div className='mb-10'>
      ...
      <header className='my-6'>
        ...
        <h1 className='py-1 font-display text-xl font-medium'>
          {isEdit ? 'Edit Rule' : 'Add Rule Mapping'}
        </h1>
      </header>
